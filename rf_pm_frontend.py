...
# This is the frontend for the RF postmortem. It displays magnitude and phase
# infromation for channels 2-4. It uses channel 1 as the reference signal
# (i.e. the RF)
...
disptime = [
    'RF postmortem for ', str(title_time.day), '/', str(title_time.month),
    '/', str(title_time.year), ' at ', str(title_time.hour), ':',
    str(title_time.minute)]
disptime = ''.join(disptime)

# case for one dataset
if len(utctime) == 1:
    disp = disptime + ' (cavity ' + str(cavity[0]) + ')'
# for multiple data sets with the same timestamp to the nearest second (i.e.
# the same incident for all cavities)
elif len(utctime) > 1 and same_ts=='True':
    disp = disptime + '\nBlue is cavity ' + str(cavity[0]) + \
        ' Green is cavity ' + str(cavity[1])
